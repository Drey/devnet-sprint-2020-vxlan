#!/usr/bin/python
#
# Cisco Connect 2018, Moscow
# This module is broken in the official repository.
# Quick fix provided to make things work.
#
    ...
    if body:
        ...
        for each in all_lines:
            if 'jp-policy' in each:
                ...
                m = re.search(r'ip pim hello-interval (\d+)', body)
                if m:
                    pim_interface['hello_interval'] = m.group(1)
            elif 'dr-priority' in each:
                m = re.search(r'ip pim dr-priority (\d+)', body)
                if m:
                    pim_interface['dr_prio'] = m.group(1)
